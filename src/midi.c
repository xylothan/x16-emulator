...
int handle_midi_event(void* data, fluid_midi_event_t* event)
{
    ...
    SDL_UnlockMutex(mrp->fifo_mutex);
    return FLUID_OK;
}
...
void midi_serial_init()
{
    ...
    if (!serial_midi_mutexes_initialized) {
        for (sel=0; sel<2; sel++) {
            mregs[sel].fifo_mutex = SDL_CreateMutex();
        }
...
void midi_serial_step(int clocks)
{
    ...
    for (sel=0; sel<2; sel++) {
        ...
        while (mregs[sel].clock < 0) {
            // process uart
            SDL_LockMutex(mregs[sel].fifo_mutex);
            ...
            SDL_UnlockMutex(mregs[sel].fifo_mutex);
